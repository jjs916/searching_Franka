...
# DBLP_ICRA_URL = DBLP_BASE_URL + 'db/conf/icra/icra2023.xml'
DBLP_PUBTITLE_URL = DBLP_BASE_URL + 'db/{pubtitle}/{ven}/{ven}{year}.xml'
...
class Venue(LazyAPIData):
    def __init__(self,pubtitle,ven,year):
        self.xml = None
        self.pubtitle = pubtitle
        self.ven = ven
        self.year = year
        super().__init__(lazy_attrs=[])

    def load_data(self):
        resp = requests.get(DBLP_PUBTITLE_URL.format(pubtitle=self.pubtitle,ven=self.ven,year=self.year))
        xml = resp.content
        self.xml = xml
        ...
        papers = [title.text for title in titles]
...
def venuesearch(pubtitle, ven, year):
    return [Venue(pubtitle,ven, year)]